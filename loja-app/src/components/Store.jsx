...
export default function Store({ currentUser, onLogout }) {
  ...
  // =========================================
  // CARROSSEL E ROLAGEM SUAVE (SMOOTH SCROLL)
  // =========================================
  ...
  const scrollToSection = (sectionId) => {
    ...
    if (element) {
      // Pega a posição do elemento e desconta o tamanho do cabeçalho (80px) para não cobrir o título
  ...
  // ESTADOS DO USUÁRIO E FOTO DE PERFIL
  ...
  // ESTADOS DOS MODAIS E AVALIAÇÕES
  ...
  const saveProfile = async (e) => {
    ...
    try {
      ...
      // Se o usuário escolheu uma foto nova, faz o upload para o Back4App
      if (avatarFile) {
      ...
      if (userToUpdate.get("avatar")) {
        setUserAvatar(userToUpdate.get("avatar").url());
      }
  ...
  const handleSubscribeNewsletter = async (e) => {
    ...
    try {
      ...
      const alreadySubscribed = await query.first();

      if (alreadySubscribed) {
        showToast("Este e-mail já está inscrito!");
      } else {
      ...
      showToast("Erro ao assinar: " + error.message);
    } finally {
  ...
  // =========================================
  // SISTEMA DE AVALIAÇÕES E MODAIS
  // =========================================
  ...
  const submitReview = async (e) => {
    ...
    } catch (error) { alert("Erro ao avaliar: " + error.message); } 
  ...
  const updateQuantity = (productId, variant, delta) => {
    setCart((prevCart) => prevCart.map(item => {
      if (item.product.id === productId && item.variant === variant) {
        ...
        const stock = item.product.get("stock");
        if (newQuantity > 0 && newQuantity <= stock) { return { ...item, quantity: newQuantity }; }
      }
      return item;
    }));
  };
  ...
  const cartTotal = cart.reduce((total, item) => {
    ...
    return total + (promo.price * item.quantity);
  }, 0);
  ...
  const submitFinalCheckout = async () => {
    ...
    try {
      ...
      for (const [productId, qtyToDeduct] of Object.entries(quantityMap)) {
        const query = new Parse.Query("Product");
        const parseProduct = await query.get(productId);
        const currentStock = parseProduct.get("stock") || 0;
        if (currentStock < qtyToDeduct) { throw new Error(`Estoque insuficiente.`); }
      ...
      for (const item of cart) {
        const promo = getActivePromo(item.product);
        orderItems.push({ productId: item.product.id, name: `${item.product.get("name")} ${item.variant ? `(${item.variant})` : ''}`, price: promo.price, quantity: item.quantity, imageUrl: item.product.get("imageUrl") });
      }

      const order = new Parse.Object("Order");
      order.set("user", Parse.User.current()); 
      order.set("total", cartTotal);
      order.set("items", orderItems);
      order.set("shippingAddress", "A Combinar via WhatsApp");
      ...
      if (paymentMethod === "pix") {
        try {
          const pixResponse = await Parse.Cloud.run("createPixPayment", {
            total: cartTotal,
            description: `Pedido #${order.id.slice(-6).toUpperCase()} - Flor e Sol`,
            email: activeUser?.get("email") || "cliente@floresol.com"
          });
          ...
          alert("Ocorreu um erro ao gerar o PIX no banco. Tente novamente.");
          await order.destroy(); 
      ...
      setCart([]); 
      localStorage.removeItem("florESol_cart"); 
  ...
  const handleWhatsAppRedirect = () => {
    const telefoneLoja = "5585999999999"; 
    const metodo = paymentMethod === "pix" ? "PIX" : "Dinheiro/Cartão na Entrega";
    const saudacao = activeUser?.get("name") ? `Olá, me chamo ${activeUser.get("name")}!` : "Olá!";
    const texto = `${saudacao} Acabei de fazer um pedido na loja (ID: ${lastOrderId}).\n\nGostaria de combinar a entrega. O método que escolhi foi: *${metodo}*.`;
  ...
  const searchResults = products.filter((p) => {
    ...
    const name = normalizeText(p.get("name"));
    const category = normalizeText(p.get("category"));
    const desc = normalizeText(p.get("description"));
    const variants = (p.get("variants") || []).map(normalizeText).join(" ");
    const searchableText = `${name} ${category} ${desc} ${variants}`;

    return searchTerms.every(term => {
      const termSingular = term.length > 3 && term.endsWith('s') ? term.slice(0, -1) : term;
      return searchableText.includes(term) || searchableText.includes(termSingular);
    });
  });
  ...
  const catalogProducts = products.filter((p) => { return selectedCategory === "Tudo" || p.get("category") === selectedCategory; });
  ...
  // =========================================
  // VARIÁVEIS DOS BANNERS (CARROSSEL)
  // =========================================
  // O código procura um array chamado "banners" no banco. Se não achar, usa os dados antigos como fallback.
  const bannersArray = storeSettings?.get("banners") || [
    {
      imageUrl: storeSettings?.get("bannerImageUrl") || "https://images.unsplash.com/photo-1490481651871-ab68de25d43d?q=80&w=2070&auto=format&fit=crop",
      tag: storeSettings?.get("bannerTag") || "Novidades",
      title: storeSettings?.get("bannerTitle") || "Coleção Essência",
      desc: storeSettings?.get("bannerDesc") || "Descubra o frescor e a elegância de peças pensadas para iluminar o seu dia a dia.",
      btn: storeSettings?.get("bannerBtn") || "Descobrir Agora",
      target: storeSettings?.get("bannerTarget") || "lancamentos" // A ID da seção para rolar
    }
  ];

  // Passador automático de slides (A cada 5 segundos)
  useEffect(() => {
    if (bannersArray.length <= 1) return; // Se for só 1 foto, não precisa passar
  ...
  const renderMosaicPromoCard = (p, index) => {
    ...
    return (
      <div key={p.id} className={`group relative overflow-hidden rounded-[24px] md:rounded-[32px] bg-card shadow-sm ${!isOutOfStock ? 'cursor-pointer' : ''} ${isFullWidth ? 'col-span-1 md:col-span-2 aspect-[4/3] md:aspect-[21/9]' : 'col-span-1 aspect-square md:aspect-[4/5]'}`}>
        ...
        <img src={p.get("imageUrl")} alt={p.get("name")} loading="lazy" className={`absolute inset-0 w-full h-full object-cover object-center transition-transform duration-1000 group-hover:scale-105 ${isOutOfStock ? "opacity-50 grayscale" : ""}`} />
        
        <button onClick={(e) => toggleFavorite(p.id, e)} className="absolute top-4 right-4 md:top-5 md:right-5 p-2.5 md:p-3 bg-white/40 backdrop-blur-md rounded-full text-white hover:scale-110 transition-transform z-20 shadow-sm">
        ...
        {!isOutOfStock && (
          <div className="absolute top-4 left-4 md:top-5 md:left-5 bg-white/90 backdrop-blur-md text-neutral-900 text-[10px] md:text-xs font-bold px-3 py-1.5 md:px-4 md:py-2 rounded-full uppercase tracking-wider shadow-sm flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20 pointer-events-none">
            <Search className="w-3 h-3 md:w-4 md:h-4" /> Ver Detalhes
          </div>
        )}
        ...
        <div className="absolute inset-x-0 bottom-0 p-5 md:p-10 flex flex-col justify-end opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-8 group-hover:translate-y-0 z-20 pointer-events-none">
          ...
          <div className="flex items-end justify-between mt-2 md:mt-4 pointer-events-auto">
            ...
            <button onClick={(e) => handleAddToCartClick(p, e)} disabled={isOutOfStock} className="shrink-0 h-10 md:h-14 px-5 md:px-8 text-sm md:text-base bg-white text-neutral-900 font-bold rounded-full flex items-center justify-center shadow-2xl hover:scale-105 transition-transform disabled:opacity-50">
  ...
  const renderNewArrivalCard = (p) => {
    ...
    return (
      <div key={p.id} className="relative shrink-0 w-[80vw] max-w-[280px] sm:max-w-[340px] md:w-full md:max-w-none snap-center pb-6 md:pb-8 pt-2">
        <div onClick={() => hasDetails && !isOutOfStock && openProductDetails(p)} className={`group relative aspect-[3/4] w-full overflow-hidden rounded-[24px] md:rounded-[32px] border border-borda shadow-sm hover:shadow-md transition-shadow duration-300 ${hasDetails && !isOutOfStock ? 'cursor-pointer' : ''}`}>
          <img src={p.get("imageUrl")} alt={p.get("name")} loading="lazy" className={`absolute inset-0 w-full h-full object-cover object-center transition-transform duration-700 group-hover:scale-105 ${isOutOfStock ? "opacity-50 grayscale" : ""}`} />
          ...
          <div className="absolute top-4 left-4 md:top-5 md:left-5 flex items-center gap-2 z-30 pointer-events-none">
            ...
            {hasDetails && !isOutOfStock && (
              <div className="bg-white/90 backdrop-blur-md text-neutral-900 text-[10px] md:text-xs font-bold px-3 py-1.5 md:px-4 md:py-2 rounded-full uppercase tracking-wider shadow-md flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <Search className="w-3 h-3 md:w-4 md:h-4" /> Detalhes
              </div>
            )}
          </div>

          <button onClick={(e) => toggleFavorite(p.id, e)} className="absolute top-4 right-4 md:top-5 md:right-5 p-2.5 md:p-3 bg-card/80 backdrop-blur-md rounded-full text-texto hover:scale-110 transition-transform z-30 shadow-sm">
          ...
          <div className="absolute inset-x-0 bottom-0 p-6 md:p-8 flex flex-col gap-2 z-20 opacity-0 translate-y-6 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 pointer-events-none">
            ...
            <button onClick={(e) => handleAddToCartClick(p, e)} disabled={isOutOfStock} className="pointer-events-auto mt-4 w-full h-12 md:h-14 text-sm md:text-base bg-texto text-card font-bold rounded-xl md:rounded-2xl flex items-center justify-center hover:opacity-90 transition-opacity disabled:opacity-50 gap-2 shadow-lg">
        ...
        <div onClick={() => hasDetails && !isOutOfStock && openProductDetails(p)} className={`group w-full aspect-[3/4] relative rounded-[24px] md:rounded-[32px] overflow-hidden border border-borda bg-card shadow-sm ${hasDetails && !isOutOfStock ? 'cursor-pointer' : ''}`}>
          ...
          <div className="absolute inset-x-0 bottom-0 p-5 md:p-6 flex flex-col justify-end opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-6 group-hover:translate-y-0 z-10 pointer-events-none">
            ...
            <div className="flex flex-col gap-3 mt-1 pointer-events-auto">
              ...
              <button onClick={(e) => handleAddToCartClick(p, e)} disabled={isOutOfStock} className="w-full h-10 md:h-12 text-sm md:text-base bg-white text-neutral-900 font-bold rounded-full flex items-center justify-center shadow-2xl hover:scale-105 transition-transform disabled:opacity-50 gap-2">
    ...
    return (
      <div key={p.id} className="group relative flex flex-col">
        <div onClick={() => hasDetails && !isOutOfStock && openProductDetails(p)} className={`aspect-[4/5] overflow-hidden rounded-[20px] md:rounded-[24px] bg-card mb-3 md:mb-4 relative border border-transparent hover:border-borda transition-colors ${hasDetails && !isOutOfStock ? 'cursor-pointer' : ''}`}>
          <img src={p.get("imageUrl")} alt={p.get("name")} loading="lazy" className={`w-full h-full object-cover object-center transition-transform duration-500 ${!isOutOfStock && "group-hover:scale-105"} ${isOutOfStock && "opacity-50 grayscale"}`} />
          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors duration-300 pointer-events-none" />
          <button onClick={(e) => toggleFavorite(p.id, e)} className="absolute top-3 right-3 md:top-4 md:right-4 p-2.5 bg-card/80 backdrop-blur-md rounded-full text-texto shadow-sm hover:scale-110 transition-transform z-20">
            <Heart className="w-4 h-4 transition-colors" fill={isFav ? "currentColor" : "none"} strokeWidth={isFav ? 0 : 2} />
          </button>
          ...
          {!isOutOfStock && (
            <div className="absolute bottom-3 left-3 right-3 md:bottom-4 md:left-4 md:right-4 translate-y-[120%] opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 z-10 pointer-events-auto">
              <button onClick={(e) => handleAddToCartClick(p, e)} className="w-full bg-btn text-btn-texto font-bold py-3 md:py-4 text-sm md:text-base rounded-xl shadow-xl hover:opacity-90 transition-colors flex items-center justify-center gap-2">
  ...
  return (
    <div className="min-h-screen bg-fundo transition-colors duration-500 font-sans relative flex flex-col text-texto">
      ...
      <header className="bg-fundo/80 backdrop-blur-2xl transition-colors duration-500 sticky top-0 z-50">
        <div className="w-full max-w-[1600px] mx-auto px-4 sm:px-6 md:px-12 lg:px-16 h-16 md:h-19 flex items-center justify-between gap-3 md:gap-4">
          ...
          {currentView === "store" && (
            <div className="flex-[2] flex justify-center items-center">
              <motion.div initial={false} animate={{ width: isSearchExpanded ? "100%" : "44px", maxWidth: isSearchExpanded ? "600px" : "44px", backgroundColor: isSearchExpanded ? "var(--cor-card)" : "transparent" }} transition={{ type: "spring", bounce: 0, duration: 0.6 }} className="relative flex items-center h-10 md:h-11 rounded-full overflow-hidden border border-transparent hover:border-borda transition-colors">
                <button onClick={() => setIsSearchExpanded(true)} className={`absolute left-0 z-10 w-10 md:w-11 h-10 md:h-11 flex items-center justify-center transition-colors duration-500 ${isSearchExpanded ? 'text-texto-sec cursor-default pointer-events-none' : 'text-texto hover:bg-texto/5 rounded-full'}`}><Search className="w-4 h-4 md:w-5 md:h-5" /></button>
                <input type="text" placeholder="O que você está procurando?" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className={`w-full h-full bg-transparent border-none py-2 pl-10 md:pl-12 pr-10 md:pr-12 focus:outline-none text-sm md:text-base text-texto transition-opacity duration-300 ${isSearchExpanded ? 'opacity-100' : 'opacity-0 cursor-pointer'}`} style={{ pointerEvents: isSearchExpanded ? 'auto' : 'none' }} />
                <AnimatePresence>
                  {isSearchExpanded && (<motion.button initial={{ opacity: 0, scale: 0.5, rotate: -45 }} animate={{ opacity: 1, scale: 1, rotate: 0 }} exit={{ opacity: 0, scale: 0.5, rotate: 45 }} transition={{ duration: 0.3 }} onClick={() => { setIsSearchExpanded(false); setSearchQuery(''); }} className="absolute right-0 z-10 w-10 md:w-11 h-10 md:h-11 flex items-center justify-center text-texto-sec hover:text-texto hover:bg-texto/5 rounded-full"><X className="w-4 h-4" /></motion.button>)}
                </AnimatePresence>
              </motion.div>
            </div>
          )}
          ...
          <div className="flex-1 flex justify-end items-center gap-1 md:gap-2 text-texto-sec">
            ...
            {/* BOTÃO DO CARRINHO (Sem scale, com dot minimalista) */}
            <button 
              onClick={() => setCurrentView("cart")} 
              className={`p-2.5 rounded-full transition-colors relative ${currentView === "cart" ? "bg-texto text-card" : "text-texto-sec hover:text-texto hover:bg-texto/5"}`}
            >
              <ShoppingBag className="w-5 h-5" />
              {cartItemsCount > 0 && (
                <span className={`absolute top-1.5 right-1.5 w-2.5 h-2.5 rounded-full border-2 ${currentView === "cart" ? "bg-card border-texto" : "bg-texto border-fundo"}`} />
              )}
            </button>
            
            {/* BOTÃO DE PERFIL (Clean, Fonte Normal, Sem Borda) */}
            <div className="relative hidden md:block ml-1">
              <button 
                onClick={() => {
                  if (currentView !== "profile") { setCurrentView("profile"); setProfileTab("data"); }
                  else { setIsUserMenuOpen(!isUserMenuOpen); }
                }} 
                className={`flex items-center gap-2 px-3 py-2.5 rounded-full transition-colors ${currentView === "profile" ? "bg-texto text-card" : "text-texto-sec hover:text-texto hover:bg-texto/5"}`}
              >
                {userAvatar ? (
                  <img src={userAvatar} alt="Perfil" className={`w-5 h-5 rounded-full object-cover border ${currentView === "profile" ? "border-card/30" : "border-borda"}`} />
                ) : (
        ...
        <AnimatePresence mode="wait">
          ...
          {currentView === "store" && (
            <motion.main 
              ...
              className="w-full max-w-[1600px] mx-auto px-4 sm:px-6 md:px-12 lg:px-16 mt-6 md:mt-8 space-y-12 md:space-y-16 mb-20"
            >
              {searchQuery ? (
                <section>
                ...
                <>
                  {/* 1. BANNER PRINCIPAL (CARROSSEL SEM ZOOM) */}
                  <section className="relative w-full h-[60vh] sm:h-[70vh] md:h-[85vh] rounded-[24px] md:rounded-[32px] overflow-hidden shadow-sm bg-fundo">
                    {bannersArray.map((banner, index) => (
                      ...
                      >
                        {/* Imagem estática, sem scale no hover para manter a sofisticação */}
                        <img src={banner.imageUrl} alt={banner.title} className="absolute inset-0 w-full h-full object-cover object-center" />
                        <div className="absolute inset-0 bg-black/30"></div>
                        
                        {/* Textos sutilmente empurrados para cima (pb-12) para melhor centralização */}
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6 pb-12 md:pb-20">
                          <span className="text-neutral-100/90 uppercase tracking-[0.3em] text-[10px] md:text-sm font-bold mb-2 md:mb-3 drop-shadow-sm">
                            {banner.tag}
                          </span>
                          <h2 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-serif italic text-neutral-100 mb-4 md:mb-6 drop-shadow-md break-words">
                            {banner.title}
                          </h2>
                          <p className="text-sm sm:text-base md:text-xl text-neutral-100/95 max-w-2xl mb-8 md:mb-10 drop-shadow-sm font-light px-4">
                            {banner.desc}
                          </p>
                          
                          {/* Botão com rolagem suave e sem hover de aumento */}
                          <button 
                            onClick={() => scrollToSection(banner.target)} 
                            className="px-8 py-3 md:px-10 md:py-4 bg-neutral-100 text-neutral-900 font-bold rounded-full hover:bg-neutral-300 transition-colors duration-300 shadow-2xl text-sm md:text-lg"
                          >
                            {banner.btn}
                          </button>
                        </div>
                    ...
                    {/* Bolinhas de navegação do Carrossel (Só aparecem se tiver mais de 1 foto) */}
                      ...
                      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 z-20">
                        {bannersArray.map((_, idx) => (
                          <button 
                            ...
                            className={`transition-all duration-500 rounded-full ${idx === currentBannerIndex ? 'w-8 h-2 bg-white' : 'w-2 h-2 bg-white/50 hover:bg-white/80'}`}
                          />
                        ))}
                      </div>
                    )}
                  ...
                  {newArrivals.length > 0 && (
                    <section id="lancamentos" className="mt-12 md:mt-16">
                      <div className="flex items-center gap-3 mb-6 px-1 md:px-0">
                        <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-btn text-btn-texto flex items-center justify-center">
                          <Sparkles className="w-3 h-3 md:w-4 md:h-4" />
                        </div>
                        <h2 className="text-xl md:text-2xl font-medium text-texto">Lançamentos</h2>
                      </div>
                      <div className="flex overflow-x-auto gap-4 md:gap-8 snap-x snap-mandatory scrollbar-hide pb-8 md:pb-10 -mx-4 px-4 sm:-mx-6 sm:px-6 md:mx-0 md:px-0 md:grid md:grid-cols-2 lg:grid-cols-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                  ...
                  <section id="mais-desejados" className="mt-12 md:mt-16">
                    <div className="flex items-center gap-3 mb-6 px-1 md:px-0">
                      <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-btn text-btn-texto flex items-center justify-center">
                  ...
                  {infoBannerActive && (
                    <section id="catalogo" className="mt-8 md:mt-24 mb-4">
                      <div className="relative w-full h-[35vh] md:h-[50vh] rounded-[24px] md:rounded-[32px] overflow-hidden shadow-sm group">
                        <img src={infoBannerImageUrl} alt={infoBannerTitle} loading="lazy" className="absolute inset-0 w-full h-full object-cover object-center group-hover:scale-105 transition-transform duration-1000" />
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6 z-10">
                          ...
                          {infoBannerBtn && (
                            <button className="px-6 py-2.5 md:px-8 md:py-3 bg-white text-neutral-900 font-bold rounded-full hover:bg-neutral-100 hover:scale-105 transition-all shadow-xl text-sm md:text-base">
                              {infoBannerBtn}
                            </button>
                          )}
                        </div>
                  ...
                  <section className="border-t border-borda pt-8 md:pt-12 mt-8 md:mt-12">
                    {categoriesList.length > 1 && (
                      <div className="flex items-center gap-3 md:gap-4 overflow-x-auto pb-6 md:pb-8 scrollbar-hide -mx-4 px-4 sm:-mx-6 sm:px-6 md:mx-0 md:px-0 md:justify-center">
                        {categoriesList.map(cat => (
                          <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-5 py-2 md:px-6 md:py-2.5 rounded-full text-xs md:text-sm font-bold transition-all duration-300 whitespace-nowrap shadow-sm border shrink-0 ${selectedCategory === cat ? "bg-btn text-btn-texto border-transparent scale-105" : "bg-card border-borda text-texto-sec hover:border-texto hover:text-texto"}`}>
                            {cat}
                          </button>
                        ))}
                      </div>
           ...
           >
             <div className="bg-card rounded-2xl md:rounded-3xl shadow-sm border border-borda overflow-hidden min-h-[400px] md:min-h-[500px] flex flex-col transition-colors duration-500">
               <div className="flex border-b border-borda flex-nowrap overflow-x-auto scrollbar-hide shrink-0 bg-fundo">
                 <button onClick={() => setProfileTab("data")} className={`flex-1 min-w-[120px] px-4 py-4 text-xs md:text-sm font-bold transition-colors whitespace-nowrap ${profileTab === "data" ? "text-texto bg-card border-b-2 border-texto" : "text-texto-sec hover:text-texto"}`}>Meus Dados</button>
                 <button onClick={() => setProfileTab("orders")} className={`flex-1 min-w-[120px] px-4 py-4 text-xs md:text-sm font-bold transition-colors whitespace-nowrap ${profileTab === "orders" ? "text-texto bg-card border-b-2 border-texto" : "text-texto-sec hover:text-texto"}`}>Histórico</button>
                 <button onClick={() => setProfileTab("favorites")} className={`flex-1 min-w-[120px] px-4 py-4 text-xs md:text-sm font-bold transition-colors whitespace-nowrap flex justify-center items-center gap-2 ${profileTab === "favorites" ? "text-texto bg-card border-b-2 border-texto" : "text-texto-sec hover:text-texto"}`}><Heart className="w-3 h-3 md:w-4 h-4" /> Favoritos</button>
               </div>
               <div className="p-5 sm:p-8 flex-1 relative overflow-hidden">
                 <AnimatePresence mode="wait">
                   
                   {/* ======================================= */}
                   {/* ABA DE DADOS (COM UPLOAD DE FOTO!) */}
                   {/* ======================================= */}
                   {profileTab === "data" && (
                     <motion.div key="tab-data" variants={tabVariants} initial="hidden" animate="visible" exit="exit" className="max-w-md mx-auto w-full">
                       ...
                       <form onSubmit={saveProfile} className="space-y-5 md:space-y-6">
                         ...
                         <button type="submit" disabled={isSavingProfile} className="w-full bg-btn text-btn-texto font-bold py-3.5 md:py-4 rounded-xl text-sm md:text-base hover:opacity-90 transition disabled:opacity-50 flex justify-center items-center gap-2 shadow-md">
                    ...
                    {checkoutStep === "success" && paymentMethod === "pix" && pixData && (
                      <div className="w-full max-w-md flex flex-col items-center">
                        ...
                        <div className="flex items-center gap-2 bg-fundo p-2 rounded-xl md:rounded-2xl w-full mb-6 md:mb-8 border border-borda min-w-0">
                          ...
                          <button onClick={handleCopyPix} className="bg-btn text-btn-texto px-4 py-3 md:px-6 md:py-4 text-[10px] md:text-xs font-bold rounded-lg md:rounded-xl hover:opacity-90 transition-colors shadow-md shrink-0">Copiar</button>
                        </div>
                        
                        <button onClick={handleWhatsAppRedirect} className="w-full py-3.5 md:py-4 bg-btn text-btn-texto font-bold rounded-xl md:rounded-2xl hover:opacity-90 transition-all shadow-xl flex items-center justify-center gap-2 md:gap-3 text-sm md:text-lg"><MessageCircle className="w-5 h-5 md:w-6 md:h-6" /> Já paguei! Enviar comprovante</button>
                      ...
                      <AnimatePresence mode="wait">
                                    ...
                                    <div className="flex-1 flex flex-col justify-between min-w-0">
                                      ...
                                      <div className="flex items-center gap-4 mt-3 md:mt-4">
                                        <div className="flex items-center gap-2 md:gap-3 bg-fundo border border-borda w-fit rounded-lg md:rounded-xl px-1.5 md:px-2 py-1">
                                          <button onClick={() => updateQuantity(item.product.id, item.variant, -1)} className="p-1 md:p-2 hover:text-texto text-texto-sec transition bg-card rounded md:rounded-lg shadow-sm border border-transparent"><Minus className="w-3 h-3 md:w-4 md:h-4" /></button>
                                          <span className="w-5 md:w-6 text-center font-bold text-xs md:text-sm text-texto">{item.quantity}</span>
                                          <button onClick={() => updateQuantity(item.product.id, item.variant, 1)} className="p-1 md:p-2 hover:text-texto text-texto-sec transition bg-card rounded md:rounded-lg shadow-sm border border-transparent"><Plus className="w-3 h-3 md:w-4 md:h-4" /></button>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                        ...
                        {checkoutStep === "payment" && (
                          <motion.div key="payment-list" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.3 }} className="space-y-4 md:space-y-6">
                            ...
                            <button onClick={() => setPaymentMethod("pix")} className={`w-full p-4 md:p-6 rounded-xl md:rounded-2xl border-2 text-left flex items-center gap-4 md:gap-6 transition-all ${paymentMethod === "pix" ? "border-texto bg-fundo shadow-md md:scale-[1.02]" : "border-borda bg-card hover:border-texto-sec hover:scale-[1.01]"}`}>
                              <div className={`p-3 md:p-4 rounded-full shrink-0 ${paymentMethod === "pix" ? "bg-btn text-btn-texto" : "bg-fundo text-texto-sec"}`}>
                                <QrCode className="w-6 h-6 md:w-8 md:h-8" />
                              </div>
                              <div>
                                <h4 className={`text-base md:text-xl font-bold ${paymentMethod === "pix" ? "text-texto" : "text-texto-sec"}`}>PIX (Na hora)</h4>
                                <p className="text-[10px] md:text-sm text-texto-sec mt-0.5 md:mt-1">Gere o código Copia e Cola na próxima tela.</p>
                              </div>
                            </button>
                            
                            <button onClick={() => setPaymentMethod("entrega")} className={`w-full p-4 md:p-6 rounded-xl md:rounded-2xl border-2 text-left flex items-center gap-4 md:gap-6 transition-all ${paymentMethod === "entrega" ? "border-texto bg-fundo shadow-md md:scale-[1.02]" : "border-borda bg-card hover:border-texto-sec hover:scale-[1.01]"}`}>
                              <div className={`p-3 md:p-4 rounded-full shrink-0 ${paymentMethod === "entrega" ? "bg-btn text-btn-texto" : "bg-fundo text-texto-sec"}`}>
                                <Truck className="w-6 h-6 md:w-8 md:h-8" />
                              </div>
                              <div>
                                <h4 className={`text-base md:text-xl font-bold ${paymentMethod === "entrega" ? "text-texto" : "text-texto-sec"}`}>Pagar na Entrega</h4>
                      ...
                      <AnimatePresence mode="wait">
                        {checkoutStep === "cart" && (
                          <motion.div key="btn-cart" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                            <button onClick={() => setCheckoutStep("payment")} disabled={cart.length === 0} className="w-full bg-btn text-btn-texto font-bold py-4 md:py-5 rounded-xl md:rounded-2xl hover:opacity-90 transition-colors shadow-lg disabled:opacity-50 flex justify-center items-center text-sm md:text-lg">
                              Ir para Pagamento
                            </button>
                          </motion.div>
                        )}
                        ...
                        {checkoutStep === "payment" && (
                          <motion.div key="btn-payment" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col gap-3 md:gap-4">
                            <button onClick={submitFinalCheckout} className="w-full bg-btn text-btn-texto font-bold py-4 md:py-5 rounded-xl md:rounded-2xl hover:opacity-90 transition-colors shadow-lg flex justify-center items-center text-sm md:text-lg gap-2">
      ...
      {/* ========================================================================= */}
      {/* MINI-MODAL DE ADIÇÃO RÁPIDA (QUICK ADD PARA ESCOLHER COR/MODELO) */}
      {/* ========================================================================= */}
      <AnimatePresence>
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setQuickAdd({ isOpen: false, product: null, selectedVariant: "" })}>
            ...
            >
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {(quickAdd.product.get("variants") || []).map(v => (
                  <button 
                    ...
                    className={`px-4 py-2.5 rounded-xl text-sm font-bold transition-all border ${quickAdd.selectedVariant === v ? 'border-texto bg-texto text-card shadow-md' : 'border-borda bg-fundo text-texto-sec hover:border-texto hover:text-texto'}`}
                  >
                    {v}
                  </button>
                ))}
              ...
              <button 
                ...
                className="w-full py-4 bg-btn text-btn-texto font-bold rounded-xl hover:opacity-90 transition-all shadow-lg flex items-center justify-center gap-2"
              >
      ...
      {/* ========================================================================= */}
      {/* MODAL GIGANTE DE DETALHES DO PRODUTO (COM AVALIAÇÕES!) */}
      {/* ========================================================================= */}
      <AnimatePresence>
        {detailedProduct && (
          <div className="fixed inset-0 z-[70] flex items-center justify-center p-0 sm:p-4 md:p-6 lg:p-10 bg-black/90 backdrop-blur-xl" onClick={() => setDetailedProduct(null)}>
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 30 }} transition={{ type: "spring", bounce: 0, duration: 0.6 }} className="bg-card w-full h-full max-w-[1400px] sm:rounded-[32px] md:rounded-[40px] overflow-hidden shadow-2xl flex flex-col md:flex-row relative transition-colors duration-500" onClick={(e) => e.stopPropagation()}>
              <button onClick={() => setDetailedProduct(null)} className="hidden md:flex absolute top-6 right-6 lg:top-8 lg:right-8 z-20 p-3 lg:p-4 bg-fundo hover:bg-borda rounded-full transition-colors text-texto shadow-xl hover:scale-105"><X className="w-5 h-5 lg:w-6 lg:h-6" /></button>
              ...
              <div className="w-full md:w-[50%] lg:w-[45%] p-6 sm:p-8 md:p-10 lg:p-12 flex flex-col flex-1 overflow-y-auto relative">
                  ...
                  <div className="mb-8 md:mb-10">
                    ...
                    <div className="flex flex-wrap gap-2 md:gap-3">
                      {(detailedProduct.get("variants") || []).map(v => (
                        <button key={v} onClick={() => setDetailedVariant(v)} className={`px-4 py-2 md:px-6 md:py-3 rounded-full text-xs md:text-sm font-bold transition-all border ${detailedVariant === v ? 'border-texto bg-texto text-card shadow-md' : 'border-borda bg-card text-texto-sec hover:border-texto'}`}>{v}</button>
                      ))}
                    </div>
                  </div>
                )}
                ...
                <div className="mb-8 md:mb-10 border-t border-borda pt-6 md:pt-8">
                  ...
                  {Parse.User.current() ? (
                    <form onSubmit={submitReview} className="bg-fundo p-4 md:p-5 rounded-xl md:rounded-2xl border border-borda">
                      ...
                      <button type="submit" disabled={isSubmittingReview} className="px-5 py-2.5 bg-btn text-btn-texto font-bold text-xs md:text-sm rounded-lg hover:opacity-90 transition-all disabled:opacity-50">
                ...
                <div className="mt-auto pt-6 md:pt-8 border-t border-borda">
                  <button onClick={() => processAddToCart(detailedProduct, detailedVariant)} disabled={detailedProduct.get("stock") <= 0} className="w-full py-4 md:py-5 lg:py-6 bg-btn text-btn-texto text-base md:text-lg font-bold rounded-xl md:rounded-2xl hover:opacity-90 transition-all shadow-xl disabled:opacity-50 disabled:shadow-none flex items-center justify-center gap-2 md:gap-3">
        ...
        <div className="w-full max-w-[1600px] mx-auto px-6 md:px-12 lg:px-16">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10 md:gap-12 mb-10 md:mb-12">
            ...
            <div>
              ...
              <ul className="space-y-2 md:space-y-3 text-xs md:text-sm text-texto-sec">
                <li><button onClick={() => setCurrentView("store")} className="hover:text-texto transition">Página Inicial</button></li>
                <li><button onClick={() => setCurrentView("profile")} className="hover:text-texto transition">Minha Conta</button></li>
              </ul>
            </div>
            <div>
              <h4 className="font-bold text-texto mb-3 md:mb-4 uppercase text-[10px] md:text-xs tracking-wider">Suporte</h4>
              <ul className="space-y-2 md:space-y-3 text-xs md:text-sm text-texto-sec">
                <li><a href="#" className="hover:text-texto transition">Fale Conosco</a></li>
                <li><a href="#" className="hover:text-texto transition">Trocas e Devoluções</a></li>
              </ul>
            </div>
            <div className="col-span-1 sm:col-span-2 md:col-span-1">
              ...
              <form onSubmit={handleSubscribeNewsletter} className="flex gap-2">
                ...
                <button type="submit" disabled={isSubscribing} className="bg-btn text-btn-texto px-3 md:px-4 py-2 rounded-lg text-xs md:text-sm font-bold hover:opacity-90 transition shrink-0 disabled:opacity-50">
          ...
          <div className="border-t border-borda pt-6 md:pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-4 md:gap-6 text-texto-sec">
              <Instagram className="w-4 h-4 md:w-5 md:h-5 hover:text-texto transition cursor-pointer" />
              <Facebook className="w-4 h-4 md:w-5 md:h-5 hover:text-texto transition cursor-pointer" />
              <Twitter className="w-4 h-4 md:w-5 md:h-5 hover:text-texto transition cursor-pointer" />
            </div>
          </div>
        </div>
      </footer>
